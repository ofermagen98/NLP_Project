...
def relation_product(x1,x2):
    ...
    n1 = int(x1.shape[1])
    n2 = int(x2.shape[1])
    O1 = tf.keras.backend.expand_dims(x1,axis=1)
    O1 = tf.keras.backend.tile(O1,multiples=(1,n2,1,1))
    O2 = tf.keras.backend.expand_dims(x2,axis=2)
    O2 = tf.keras.backend.tile(O2,multiples=(1,1,n1,1))
