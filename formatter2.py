...
def get_imgs(img_id):
    ...
    features = [
        fix("detection_classes"),
        fix("detection_scores"),
        OBJ["detection_boxes"],
    ]
    for A in features:
        print(A.shape)
...
def get_example(ex):
    ...
    if ID + "-img0.png" not in ID2images_path or not os.path.isfile(
        ID2boxes_path[ID + "-img0.png"]
    ):
        # print(ID + "-img0.png")
        return None
    if ID + "-img1.png" not in ID2images_path or not os.path.isfile(
        ID2boxes_path[ID + "-img1.png"]
    ):
        # print(ID + "-img1.png")
        return None
...
ID2boxes_path = (
    "/Users/ofermagen/Coding/NLP_Project/get_boxes/google_train_ID2path.json"
)
...
with open(ID2images_path, "r") as f:
    ID2images_path = json.load(f)
with open(ID2boxes_path, "r") as f:
    ID2boxes_path = json.load(f)
